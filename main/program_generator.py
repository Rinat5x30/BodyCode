...
exercise_list = {
    "грудь": ["Жим штанги лежа", "Жим штанги на наклонной скамье", "Сведение рук", "Кроссовер в тренажёре", "Жим гантелей лёжа"],
    "спина": ["Тяга верхнего блока", "Тяга гантели в наклоне", "Горизонтальная тяга"],
    "квадрицепсы": ["Приседания со штангой", "Выпады с гантелями", "Разгибание ног", "Жим ног в тренажёре", "Зашагивания на платформу"],
    "бицепс бедра": ["Сгибания ног лежа", "Румынская тяга", "Сгибания ног стоя"],
    "средняя дельта": ["Тяга к подбородку с EZ-грифом", "Махи гантелями в стороны", "Разведение гантелей в стороны"],
    "бицепс": ["Подъем гантелей на бицепс", "Подъем EZ-грифа на бицепс"],
    "трицепс": ["Разгибание рук на блоке", "Французский жим гантели сидя", "Разгибание рук с гантелью над головой"],
    "пресс": ["Скручивания", "Подъем ног в висе", "Планка", "Боковая планка"],
...
def get_available_exercises_for_group(group: str, contraindications: list[str], count: int = 2) -> list[str]:
    result: list[str] = []
    for ex in exercise_list.get(group, []):
        if len(result) >= count:
            break
        if apply_contra_filters([ex], contraindications):
            result.append(ex)
    return result
